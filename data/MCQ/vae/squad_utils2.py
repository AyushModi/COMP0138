...
class SquadExample(object):
    ...
    def __init__(self,
        ...
        self.distractor_text = distractor_text
    ...
    def __repr__(self):
        ...
        s += ", distractor_text: %s" % self.distractor_text
...
def convert_examples_to_features_answer_id(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in tqdm(enumerate(examples), total=len(examples)):
        query_tokens = tokenizer.tokenize(example.question_text)
        distractor_tokens = tokenizer.tokenize(example.distractor_text)

        if len(query_tokens) > max_query_length:
            query_tokens = query_tokens[0:max_query_length]
        if len(distractor_tokens) > max_distractor_length:
            distractor_tokens = distractor_tokens[0:max_distractor_length]
        ...
        # The -4 accounts for [CLS], [SEP], [SEP] and [SEP]
        max_tokens_for_doc = max_seq_length - len(query_tokens)-len(distractor_tokens) - 4
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            if is_training and not example.is_impossible:
                ...
                if out_of_span:
                    ...
                    doc_offset = len(query_tokens) + 2 + len(distractor_tokens) + 1
                    start_position = tok_start_position - doc_start + doc_offset
                    end_position = tok_end_position - doc_start + doc_offset
            ...
            d_tokens = deepcopy(distractor_tokens)[:max_distractor_length - 2]
            d_tokens.insert(0, "[CLS]")
            d_tokens.append("[SEP]")
            d_ids = tokenizer.convert_tokens_to_ids(d_tokens)
            c_ids = tokenizer.convert_tokens_to_ids(context_tokens)
            ...
            while len(d_ids) < max_distractor_length:
                d_ids.append(0)
            ...
            for qa in paragraph["qas"]:
                ...
                if is_training:
                    ...
                    if not is_impossible:
                        distractor_text = qa["distractors"][0]
                        answer = qa["answers"][0]
                        orig_answer_text = answer["text"]
            ...
            for start_index in start_indexes:
                for end_index in end_indexes:
                    if noq_position:
                        ...
                        d_ids = feature.d_ids
                        d_len = np.sum(np.sign(d_ids))
                        noq_start_index = start_index
                        noq_end_index = end_index
                        start_index = start_index + q_len - 1 + d_len
                        end_index = end_index + q_len - 1 + d_len
