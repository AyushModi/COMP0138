...
from squad_utils2 import (convert_examples_to_features_answer_id,
...
def get_squad_data_loader(tokenizer, file, shuffle, args):
    ...
    all_d_ids = torch.tensor([f.d_ids for f in features], dtype=torch.long)
    ...
    all_data = TensorDataset(all_c_ids, all_q_ids, all_a_ids, all_start_positions, all_end_positions,all_d_ids)
...
def batch_to_device(batch, device):
    ...
    c_ids, q_ids, a_ids, start_positions, end_positions, d_ids = batch
    ...
    d_len = torch.sum(torch.sign(d_ids), 1)
    max_d_len = torch.max(d_len)
    d_ids = d_ids[:, :max_d_len]

    return c_ids, q_ids, a_ids, start_positions, end_positions, d_ids
