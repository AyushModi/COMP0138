...
def main(args):
    ...
    for epoch in trange(int(args.epochs), desc="Epoch", position=0):
        for batch in tqdm(train_loader, desc="Train iter", leave=False, position=1):
            c_ids, q_ids, a_ids, start_positions, end_positions, all_qtype_ids \
            = batch_to_device(batch, args.device)
            
            trainer.train(c_ids, q_ids, a_ids, start_positions, end_positions, all_qtype_ids)
            
            str1 = 'Q REC : {:06.4f} A REC : {:06.4f} Q TYPE: {:06.4f}'
            str2 = 'ZQ KL : {:06.4f} ZA KL : {:06.4f} INFO : {:06.4f}'
            str1 = str1.format(float(trainer.loss_q_rec), float(trainer.loss_a_rec), float(trainer.loss_qtype))
        ...
        if epoch > 10:
            ...
            if f1 > best_f1:
                best_f1 = f1
                trainer.save(os.path.join(args.model_dir, f"best_f1_model.pt"))
            if bleu > best_bleu:
                best_bleu = bleu
                trainer.save(os.path.join(args.model_dir, f"best_bleu_model.pt"))
