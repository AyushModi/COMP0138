...
def eval_vae(epoch, args, trainer, eval_data):
    ...
    for batch in tqdm(eval_loader, desc="Eval iter", leave=False, position=4):
        c_ids, q_ids, a_ids, start, end, qtype_ids = batch_to_device(batch, args.device)
        ...
        posterior_z_prob = trainer.generate_posterior(c_ids, q_ids, a_ids, qtype_ids)

        batch_start_logits, batch_end_logits \
        = trainer.generate_answer_logits(c_ids, q_ids, a_ids, qtype_ids)
        ...
        prior_z_prob = trainer.generate_prior(c_ids, qtype_ids)
