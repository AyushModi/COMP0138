...
class InputFeatures(object):
    ...
    def __init__(self,
        ...
        self.d1_ids = d1_ids
        self.d1_tokens = d1_tokens
        self.d2_ids = d2_ids
        self.d2_tokens = d2_tokens
        self.d3_ids = d3_ids
        self.d3_tokens = d3_tokens
...
def convert_examples_to_features_answer_id(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in tqdm(enumerate(examples), total=len(examples)):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            d_tokens1 = deepcopy(distractor_tokens1)[:max_distractor_length - 1]
            d_tokens2 = deepcopy(distractor_tokens2)[:max_distractor_length - 1]
            d_tokens3 = deepcopy(distractor_tokens3)[:max_distractor_length - 1]
            ...
            ### Above comment was a mistake. Should not have done that.
    ...
    for entry in tqdm(input_data, total=len(input_data)):
        ...
        for paragraph in paragraphs:
            ...
            for qa in paragraph["qas"]:
                qas_id = qa["id"]
                question_text = qa["question"]
                distractor_texts = qa["distractors"]
                start_position = None
                end_position = None
                orig_answer_text = None
                is_impossible = False
            ...
            for start_index in start_indexes:
                for end_index in end_indexes:
                    if noq_position:
                        ...
                        d1_ids = feature.d1_ids
                        d2_ids = feature.d2_ids
                        d3_ids = feature.d3_ids
                        d_len = np.sum(np.sign(d1_ids))+np.sum(np.sign(d2_ids))+np.sum(np.sign(d3_ids))
