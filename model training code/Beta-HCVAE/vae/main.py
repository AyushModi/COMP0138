...
def main(args):
    print("Lambda_KL is: ", args.lambda_kl)
    ...
    beta_str = f"{float(args.lambda_kl):.2f}".replace(".","_")
    ...
    for epoch in trange(int(args.epochs), desc="Epoch", position=0):
        for batch in tqdm(train_loader, desc="Train iter", leave=False, position=1):
            ...
            anneal_kl = float(args.lambda_kl)*float(epoch) / (float(args.epochs)-1)
            trainer.train(c_ids, q_ids, a_ids, start_positions, end_positions, anneal_kl)
            
            str1 = 'Q REC : {:06.4f} A REC : {:06.4f} ANNEAL : {:.4f}'
            str2 = 'ZQ KL : {:06.4f} ZA KL : {:06.4f} INFO : {:06.4f}'
            str1 = str1.format(float(trainer.loss_q_rec), float(trainer.loss_a_rec),float(anneal_kl))
        ...
        if epoch > 10:
            ...
            if f1 > best_f1:
                best_f1 = f1
                trainer.save(os.path.join(args.model_dir, f"best_f1_model{beta_str}.pt"))
            if bleu > best_bleu:
                best_bleu = bleu
                trainer.save(os.path.join(args.model_dir, f"best_bleu_model{beta_str}.pt"))
